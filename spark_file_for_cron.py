...
output_dir = os.path.join(BASE_DIR, "yn_processed_kpis")
os.makedirs(output_dir, exist_ok=True)

def get_output_dir(dir_name):
    # Define base path assuming /app is your working directory inside the container
    final_output_dir = os.path.join(BASE_DIR, "yn_processed_kpis", dir_name)
...
# docker run -d --name kpi-cron -v D:/kafka-docker-project/yn_project_cron_only:/app spark-kpi-cron -- Final run cron command
# docker logs -f kpi-cron

# commands to stop the cron job
# docker stop kpi-cron
# docker rm -f kpi-cron
# docker logs -f kpi-cron